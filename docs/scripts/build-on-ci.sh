...
# we only want to build documentation from the "main" branch
...
# TRAVIS_BRANCH=main, but TRAVIS_PULL_REQUEST_BRANCH=the-branch
# and TRAVIS_PULL_REQUEST=42 where 42 is a pull request number
if [ "$TRAVIS_BRANCH" = "main" -a "$TRAVIS_PULL_REQUEST" = "false" -a "$LISP" = "ccl" ]; then
